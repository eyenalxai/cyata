...
export const selectUserByUsername = (trx: typeof db, username: string) => {
	return ResultAsync.fromPromise(trx.select().from(users).where(eq(users.username, username)), (e) =>
		getErrorMessage(e, "Failed to get user by username")
	).andThen((users) => (users.length > 0 ? okAsync(users[0]) : errAsync("User not found")))
}

export const selectUserByUuid = (trx: typeof db, uuid: string) => {
	return ResultAsync.fromPromise(trx.select().from(users).where(eq(users.uuid, uuid)), (e) =>
		getErrorMessage(e, "Failed to get user by username")
	).andThen((users) => (users.length > 0 ? okAsync(users[0]) : errAsync("User not found")))
}
